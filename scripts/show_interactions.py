...
if __name__=="__main__":
    with open("../data/traces_filtered.json") as f, open("service_ip.json") as ip_mapping:
        traces = json.load(f)
        ip_mapping = json.load(ip_mapping)
        service_index_map={"frontend":0,  "adservice":1, "recommendationservice":2, "cartservice":4, "checkoutservice":8, "paymentservice":7, "shippingservice":5, "emailservice":9, "currencyservice":6, "productcatalogservice":3}
            ...
            for resourceSpan in trace["resourceSpans"]:
                ...
                for attribute in resourceSpan["resource"]["attributes"]:
                    if attribute["key"]=="service.name":
                        ...
                        break
                if resource_name == "":
                    raise Exception("service.name attribute not found in resource attributes\n"+str(resourceSpan["resource"]["attributes"]))
                ...
                for scopeSpan in resourceSpan["scopeSpans"]:
                    for span in scopeSpan["spans"]:
                        ...
                        for attribute in span["attributes"]:
                            if span_kind==2:
                                if attribute["key"]=="network.peer.address" or attribute["key"]=="client.address" or attribute["key"]=="net.peer.ip":
                                    client_address = attribute["value"]["stringValue"]
                                    if client_address in ip_mapping:
                                        client_address = ip_mapping[client_address]
                                    if client_address in service_index_map:
                                        client_index = service_index_map[client_address]
                                        n_interactions[client_index][service_index_map[resource_name]]+=1
                                ...
                                if attribute["key"]=="server.address":
                                    ...
                                    if server_address in ip_mapping:
                                        server_address = ip_mapping[server_address]
                                    if server_address in service_index_map:
                                        server_index = service_index_map[server_address]
                                        n_interactions[service_index_map[resource_name]][server_index]+=1
                                    ...
                                    for service in service_index_map.keys():
                                        if service in service_name.lower():
                                            n_interactions[service_index_map[resource_name]][service_index_map[service]]+=1
                                            unknown_server=False
                                            break
                        if unknown_client:
        ...
        for key, value in service_index_map.items():
            print(f"{key}: {n_unknown_callers[value]}")
        print("Unknown callees per service:")
        for key, value in service_index_map.items():
            print(f"{key}: {n_unknown_callees[value]}")